...
public:
  ...
  void set_serialized_data(pybind11::bytes & data) {
    ...
    auto ret = rcutils_uint8_array_init(&uint8_array, buffer_length, &allocator);
  ...
  pybind11::bytes get_serialized_data() {
    std::string tmp_serialized_data(serialized_data.buffer,
      serialized_data.buffer + serialized_data.buffer_length);
...
public:
  Writer(const std::string & writer_class)
  {
    if (!writer_class.compare("SequentialWriter")) {
      ...
      throw std::runtime_error{"Writer class type " + writer_class + " not supported."};
    }
  }
...
PYBIND11_MODULE(_rosbag2_py, m) {
  ...
  pybind11::class_<rosbag2_py::SerializedBagMessage, std::shared_ptr<rosbag2_py::SerializedBagMessage>>(m, "SerializedBagMessage")
  .def(pybind11::init())
  .def_readwrite("topic_name", &rosbag2_py::SerializedBagMessage::topic_name)
  .def_property("serialized_data", &rosbag2_py::SerializedBagMessage::get_serialized_data, &rosbag2_py::SerializedBagMessage::set_serialized_data)
  .def_readwrite("time_stamp", &rosbag2_py::SerializedBagMessage::time_stamp);
