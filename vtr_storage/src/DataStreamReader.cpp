...
namespace vtr {
namespace storage {
DataStreamReader::DataStreamReader(const std::string &data_directory,
                                   const std::string &stream_name)
    : DataStreamBase(data_directory, stream_name) {}

DataStreamReader::~DataStreamReader() { close(); }
...
std::shared_ptr<TestMsgT> DataStreamReader::readAtIndex(int32_t index) {
  ...
  serialization_.deserialize_message(&extracted_serialized_msg,
                                     extracted_test_msg.get());
  return extracted_test_msg;
}

std::shared_ptr<TestMsgT> DataStreamReader::readAtTimestamp(
...
std::shared_ptr<std::vector<std::shared_ptr<TestMsgT>>>
DataStreamReader::readAtIndexRange(int32_t index_begin, int32_t index_end) {
  openAndGetMessageType();
  ...
  auto deserialized_bag_message_vector =
      std::make_shared<std::vector<std::shared_ptr<TestMsgT>>>();
  for (auto bag_message : *bag_message_vector) {
    ...
    deserialized_bag_message_vector->push_back(
        extracted_test_msg);  // ToDo: reserve the vector instead of pushing
                              // back
  }
...
std::shared_ptr<std::vector<std::shared_ptr<TestMsgT>>>
DataStreamReader::readAtTimestampRange(rcutils_time_point_value_t time_begin,
