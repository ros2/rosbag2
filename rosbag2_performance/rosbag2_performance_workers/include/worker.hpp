...
public:
        ...
        : Node(name)
    {
        ...
        benchmark_path = parameters_client->get_parameter(std::string("benchmark_path"), std::string(""));
        ...
        delay_timer = this->create_wall_timer(std::chrono::milliseconds(delay), std::bind(&Worker::delay_callback, this));
    }
...
private:
    void delay_callback() noexcept
    {
        ...
        timer = this->create_wall_timer(std::chrono::milliseconds(1000 / frequency), std::bind(&Worker::timer_callback, this));
        delay_timer->cancel();
    }
