...
void TransientLocalMessagesCache::push(const std::string & topic_name, MessageSharedPtr message)
{
  std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<TransientLocalMessagesCache::MessageSharedPtr>
TransientLocalMessagesCache::get_messages_sorted_by_timestamp() const
{
  std::vector<MessageSharedPtr> messages;
  {
    std::lock_guard<std::mutex> lock(mutex_);
