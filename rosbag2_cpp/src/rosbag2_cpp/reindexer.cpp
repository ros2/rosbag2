...
/// Foxy compatibility. Adapt for lack of https://github.com/ros2/rcutils/pull/323
namespace
{
extern "C"
{
...
rcutils_dir_iter_start(const char * directory_path, const rcutils_allocator_t allocator)
{
  ...
  rcutils_dir_iter_t * iter = (rcutils_dir_iter_t *)allocator.zero_allocate(  // NOLINT
  ...
  rcutils_dir_iter_state_t * state = (rcutils_dir_iter_state_t *)allocator.zero_allocate(  // NOLINT
...
rcutils_dir_iter_next(rcutils_dir_iter_t * iter)
{
  ...
  rcutils_dir_iter_state_t * state = (rcutils_dir_iter_state_t *)iter->state;  // NOLINT
  ...
  std::vector<rcpputils::fs::path> & output)
{
  ...
  do {
    ...
    if (std::regex_match(found_file.string(), regex_rule)) {
      auto full_path = base / found_file;
...
void Reindexer::reindex(const rosbag2_storage::StorageOptions & storage_options)
{
  ...
  ROSBAG2_CPP_LOG_INFO_STREAM("Beginning reindexing bag in directory: " << base_folder_.string());
