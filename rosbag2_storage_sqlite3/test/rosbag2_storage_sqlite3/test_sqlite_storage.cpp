...
TEST_F(StorageTestFixture, read_next_returns_filtered_messages_topics_regex_to_exclude) {
  ...
  rosbag2_storage::StorageFilter storage_filter;
  storage_filter.regex_to_exclude = "prefix.*";
  readable_storage->set_filter(storage_filter);
...
TEST_F(StorageTestFixture, read_next_returns_filtered_messages_complex_condition)
{
  std::vector<std::tuple<std::string, int64_t, std::string, std::string, std::string>>
  string_messages =
  {std::make_tuple("topic1 message", 1, "topic1", "", ""),
    std::make_tuple("service event topic 1 message", 2, "service_topic1/_service_event", "", ""),
    std::make_tuple("topic2 message", 3, "topic2", "", ""),
    std::make_tuple("service event topic 2 message", 4, "service_topic2/_service_event", "", ""),
    std::make_tuple("topic3 message", 5, "topic3", "", "")};
  ...
  auto open_storage = [&]() {
      ...
      readable_storage->open({db_filename, kPluginID});
      return readable_storage;
    };

  // Set topic list and regex for service
  {
    auto readable_storage = open_storage();
