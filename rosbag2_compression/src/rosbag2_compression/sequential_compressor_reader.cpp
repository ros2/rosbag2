...
void SequentialCompressorReader::reset()
{
  storage_.reset();
}

void SequentialCompressorReader::open_storage()
{
...
void SequentialCompressorReader::setup_compression()
{
...
void SequentialCompressorReader::open(
...
bool SequentialCompressorReader::has_next()
{
  if (storage_) {
...
std::shared_ptr<rosbag2_storage::SerializedBagMessage> SequentialCompressorReader::read_next()
{
  if (storage_) {
...
std::vector<rosbag2_storage::TopicMetadata> SequentialCompressorReader::get_all_topics_and_types()
{
  if (storage_) {
...
std::string SequentialCompressorReader::get_current_uri() const
{
...
void SequentialCompressorReader::check_topics_serialization_formats(
...
void SequentialCompressorReader::check_converter_serialization_format(
