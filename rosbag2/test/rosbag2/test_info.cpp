...
TEST_F(TemporaryDirectoryFixture, read_metadata_supports_version_2) {
  ...
  {
    const auto actual_relative_file_paths = metadata.relative_file_paths;
    const auto expected_relative_file_paths = std::vector<std::string>{
      "some_relative_path",
      "some_other_relative_path"
    };

    EXPECT_EQ(actual_relative_file_paths, expected_relative_file_paths);
  }

  EXPECT_EQ(metadata.duration, std::chrono::nanoseconds{100});

  {
    const auto expected_starting_time = std::chrono::time_point<std::chrono::high_resolution_clock>{
      std::chrono::nanoseconds{1000000}
    };

    EXPECT_EQ(metadata.starting_time, expected_starting_time);
  }

  EXPECT_EQ(metadata.message_count, 500u);
