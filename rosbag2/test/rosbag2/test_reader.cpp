...
public:
  ReaderTest()
  {
    ...
    reader_ = std::make_unique<rosbag2::Reader>(
...
// TODO(piraka9011): Refactor names to follow googletest's CamelCase convention.
// Reason: https://bit.ly/2ORi5kP
TEST_F(ReaderTest, read_next_uses_converters_to_convert_serialization_format) {
...
TEST_F(ReaderTest, open_throws_error_if_converter_plugin_does_not_exist) {
  std::string storage_serialization_format = "rmw1_format";
  std::string output_format = "rmw2_format";
  set_storage_serialization_format(storage_serialization_format);
...
TEST_F(ReaderTest, reset_clears_storage)
{
  std::string storage_serialization_format = "rmw1_format";
  set_storage_serialization_format(storage_serialization_format);
  reader_->open(rosbag2::StorageOptions(), {"", storage_serialization_format});
  reader_->reset();
  // has_storage is private
  EXPECT_ANY_THROW(reader_->read_next());
}

TEST_F(ReaderTest, has_next_throws_if_no_storage)
{
  std::string storage_serialization_format = "rmw1_format";
  set_storage_serialization_format(storage_serialization_format);
  reader_->open(rosbag2::StorageOptions(), {"", storage_serialization_format});
  // Resetting because test fixture expects storage to be opened at least once
  reader_->reset();
  EXPECT_ANY_THROW(reader_->has_next());
}

TEST_F(ReaderTest, read_next_throws_if_no_storage)
{
  std::string storage_serialization_format = "rmw1_format";
  set_storage_serialization_format(storage_serialization_format);
  reader_->open(rosbag2::StorageOptions(), {"", storage_serialization_format});
...
TEST_F(ReaderTest, get_all_topics_and_types_throws_if_no_storage)
{
  std::string storage_serialization_format = "rmw1_format";
  set_storage_serialization_format(storage_serialization_format);
  reader_->open(rosbag2::StorageOptions(), {"", storage_serialization_format});
