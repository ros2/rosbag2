...
TEST_F(PlayEndToEndTestFixture, play_end_to_end_test_for_bag_v2) {
  sub_->add_subscription<std_msgs::msg::String>("/string_test_topic", 2);
  sub_->add_subscription<std_msgs::msg::Int32>("/int_test_topic", 2);

  auto subscription_future = sub_->spin_subscriptions();

  auto exit_code = execute_and_wait_until_completion(
    "ros2 bag play rosbag_v2_test.bag -s rosbag_v2", database_path_);

  subscription_future.get();

  auto string_messages = sub_->get_received_messages<std_msgs::msg::String>("/string_test_topic");
  auto int_messages = sub_->get_received_messages<std_msgs::msg::Int32>("/int_test_topic");

  EXPECT_THAT(exit_code, Eq(EXIT_SUCCESS));

  EXPECT_THAT(string_messages, SizeIs(Ge(2u)));
  EXPECT_THAT(string_messages,
    Each(Pointee(Field(&std_msgs::msg::String::data, "test"))));

  EXPECT_THAT(int_messages, SizeIs(2));
  EXPECT_THAT(int_messages[0]->data, Eq(119283));
  EXPECT_THAT(int_messages[1]->data, Eq(-37));
}

TEST_F(PlayEndToEndTestFixture, play_fails_gracefully_if_rosbag_v2_storage_id_is_not_specified) {
  internal::CaptureStderr();
  auto exit_code =
    execute_and_wait_until_completion("ros2 bag play rosbag_v2_test.bag", database_path_);
