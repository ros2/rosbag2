...
#ifndef ROSBAG2_TRANSPORT__RECORDER_DELAYED_ACTION_RUNNER_HPP_
#define ROSBAG2_TRANSPORT__RECORDER_DELAYED_ACTION_RUNNER_HPP_
...
class RecorderDelayedActionRunner
{
public:
  /// @brief Construct a runner bound to the provided node.
  /// @param node Node used for logging and to query current time.
  explicit RecorderDelayedActionRunner(rclcpp::Node * node);
  ~RecorderDelayedActionRunner();
...
private:
  std::unique_ptr<RecorderDelayedActionRunnerImpl> pimpl_;
};

}  // namespace rosbag2_transport

#endif  // ROSBAG2_TRANSPORT__RECORDER_DELAYED_ACTION_RUNNER_HPP_
