  ...
  const rclcpp::Logger & logger)
{
  ...
  if (topic.version <=8){
    const auto offered_qos_profiles = profiles_yaml.as<std::vector<rosbag2_transport::Rosbag2QoS_v<8>>>();
    return rosbag2_transport::Rosbag2QoS_v<8>::adapt_offer_to_recorded_offers(topic.name, offered_qos_profiles);
  } else {
    const auto offered_qos_profiles = profiles_yaml.as<std::vector<rosbag2_transport::Rosbag2QoS_v<9>>>();
    return rosbag2_transport::Rosbag2QoS_v<9>::adapt_offer_to_recorded_offers(topic.name, offered_qos_profiles);
  }
}
}  // namespace
