  ...
  const std::shared_ptr<uint8_t[]> & type_erased_send_goal_service_event)
{
  ...
  if (request_member.size_function(request_sequence_ptr) > 0) {
      ...
      [this](rclcpp_action::GenericClientGoalHandle::SharedPtr goal_handle) {
        ...
        if (goal_handle) {
          ...
          if (need_to_cancel) {
            ...
            // The goal id need not to be add to goal_id_to_goal_handle_map_.
            return;
          }

          // Check if the Goal ID needs to get result.
          bool need_to_get_result = false;
          {
            std::lock_guard<std::mutex> lock(goal_ids_to_postpone_send_result_mutex_);
            if (goal_ids_to_postpone_send_result_.count(goal_id) > 0) {
              goal_ids_to_postpone_send_result_.erase(goal_id);
              need_to_get_result = true;
            }
          }
          if (need_to_get_result) {
    ...
    auto result_callback =
      [this](const rclcpp_action::GenericClientGoalHandle::WrappedResult & result) {
  ...
  const std::shared_ptr<uint8_t[]> & type_erased_cancel_goal_service_event)
{
  ...
  if (!get_goal_id_from_cancel_goal_service_event(
    type_erased_cancel_goal_service_event, goal_id))
  {
    RCLCPP_WARN(
      logger_,
  ...
  {
    std::lock_guard<std::mutex> lock(goal_id_to_goal_handle_map_mutex_);
    auto it = goal_id_to_goal_handle_map_.find(goal_id);
    if (it != goal_id_to_goal_handle_map_.end()) {
      goal_handle = it->second;
      // Remove this goal handle since goal is going to be canceled
      goal_id_to_goal_handle_map_.erase(it);
    } else {
      // Record this Goal ID, and process the cancel request after the goal is accepted
      {
        std::lock_guard<std::mutex> lock(goal_ids_to_postpone_send_cancel_mutex_);
        goal_ids_to_postpone_send_cancel_.insert(goal_id);
      }
      RCLCPP_DEBUG(
        logger_,
  ...
  const std::shared_ptr<uint8_t[]> & type_erased_get_result_service_event)
{
  ...
  GoalID goal_id;
  if (!get_goal_id_from_get_result_service_event(type_erased_get_result_service_event, goal_id)) {
    RCLCPP_WARN(
      logger_,
  ...
  rclcpp_action::GenericClientGoalHandle::SharedPtr goal_handle;
  {
    ...
    if (it != goal_id_to_goal_handle_map_.end()) {
      ...
      {
        std::lock_guard<std::mutex> lock(goal_ids_to_postpone_send_result_mutex_);
        goal_ids_to_postpone_send_result_.insert(goal_id);
      }
      RCLCPP_DEBUG(
        logger_,
  ...
  auto result_callback =
    [this, goal_id](const rclcpp_action::GenericClientGoalHandle::WrappedResult & result) {
      (void) result;
      std::lock_guard<std::mutex> lock(goal_id_to_goal_handle_map_mutex_);
      goal_id_to_goal_handle_map_.erase(goal_id);
    };
